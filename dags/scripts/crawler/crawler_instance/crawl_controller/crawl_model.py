...
from scripts.crawler.constants.constant import CRAWL_SETTINGS_CONSTANTS, RAW_PATH_CONSTANTS
from scripts.crawler.constants.strings import MANAGE_CRAWLER_MESSAGES
from scripts.crawler.crawler_instance.crawl_controller.crawl_enums import CRAWL_MODEL_COMMANDS
from scripts.crawler.crawler_instance.genbot_service import genbot_controller
from scripts.crawler.crawler_instance.helper_services.helper_method import helper_method
...
from scripts.crawler.crawler_services.crawler_services.mongo_manager.mongo_controller import mongo_controller
from scripts.crawler.crawler_services.crawler_services.mongo_manager.mongo_enums import MONGODB_COMMANDS, MONGO_CRUD
...
from scripts.crawler.crawler_services.crawler_services.dynamo_manager.dynamo_controller import dynamo_controller
from scripts.crawler.crawler_services.crawler_services.dynamo_manager.dynamo_enums import DYNAMO_CRUD, DYNAMO_COMMANDS
...
class crawl_model(request_handler):
    ...
    def __install_live_url(self):
        mongo_response = mongo_controller.get_instance().invoke_trigger(MONGO_CRUD.S_READ,
                                                                        [MONGODB_COMMANDS.S_GET_CRAWLABLE_URL_DATA,
                                                                         [None], [None]])
        m_live_url_list = []
        ...
        m_response = m_request_handler.get(CRAWL_SETTINGS_CONSTANTS.S_START_URL, headers=headers,
                                           timeout=CRAWL_SETTINGS_CONSTANTS.S_URL_TIMEOUT, proxies=proxies,
                                           allow_redirects=True, )
        m_updated_url_list = []

        for m_server_url in m_response.text.splitlines():
            if helper_method.is_uri_validator(m_server_url) and helper_method.on_clean_url(
                    m_server_url) not in m_live_url_list:
                ...
                mongo_controller.get_instance().invoke_trigger(MONGO_CRUD.S_UPDATE,
                                                               [MONGODB_COMMANDS.S_INSTALL_CRAWLABLE_URL,
                                                                [helper_method.on_clean_url(m_server_url)], [True]])
                m_updated_url_list.append(m_server_url)

        mongo_controller.get_instance().invoke_trigger(MONGO_CRUD.S_DELETE,
                                                       [MONGODB_COMMANDS.S_REMOVE_DEAD_CRAWLABLE_URL, [m_live_url_list],
                                                        [None]])
    ...
    def __start_docker_request(self, p_fetched_url_list):
        # genbot_controller.celery_genbot_instance(p_fetched_url_list[0])
        self.__start_direct_request(p_fetched_url_list)
    ...
    def __on_load_analytics(self, weekly):
        m_file = open("analytics.txt", "w")
        response, result = dynamo_controller.get_instance().invoke_trigger(DYNAMO_CRUD.S_READ, [DYNAMO_COMMANDS.S_GET_ALL_ARTICLE, [weekly], [True]])
        ...
        for item in result:
            m_time = int(float(item["m_content_time"]))
            m_current_time = int(float(time.time()))
            if weekly:
                if m_current_time - m_time > 432000:
                    continue
            else:
                if m_current_time - m_time > 3600:
                    continue
    ...
    def __run_crawler(self):
        ...
        if APP_STATUS.DOCKERIZED_RUN:
            self.__start_docker_request(m_live_url_list)
        else:
            self.__start_direct_request(m_live_url_list)

    def invoke_trigger(self, p_command, p_data=None):
        if p_command == CRAWL_MODEL_COMMANDS.S_INIT:
            self.__init_crawler()
        if p_command == CRAWL_MODEL_COMMANDS.S_RUN_CRAWLER:
            self.__run_crawler()
        if p_command == CRAWL_MODEL_COMMANDS.S_LOAD_ANALYTICS:
            self.__on_load_analytics(False)
        if p_command == CRAWL_MODEL_COMMANDS.S_LOAD_ANALYTICS_WEEKLY:
            self.__on_load_analytics(True)
