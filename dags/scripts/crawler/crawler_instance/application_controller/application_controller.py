...
from scripts.crawler.crawler_instance.crawl_controller.crawl_enums import CRAWL_CONTROLLER_COMMANDS
...
from scripts.crawler.crawler_services.crawler_services.topic_manager.topic_classifier_controller import topic_classifier_controller
from scripts.crawler.crawler_services.crawler_services.topic_manager.topic_classifier_enums import TOPIC_CLASSFIER_COMMANDS
from scripts.crawler.crawler_instance.crawl_controller.crawl_controller import crawl_controller
...
class application_controller(request_handler, ABC):
    ...
    def __on_init(self):
        ...
        self.__m_crawl_controller.invoke_trigger(CRAWL_CONTROLLER_COMMANDS.S_INIT_CRAWLER)
    ...
    def invoke_triggers(self, p_command):
        if p_command == APPICATION_COMMANDS.S_START_APPLICATION_DIRECT:
            ...
            return self.__on_start()
        elif p_command == APPICATION_COMMANDS.S_INIT_APPLICATION_DOCKERISED:
            return self.__on_init()
        elif p_command == APPICATION_COMMANDS.S_LOAD_ANALYTICS:
            self.__m_crawl_controller.invoke_trigger(CRAWL_CONTROLLER_COMMANDS.S_LOAD_ANALYTICS)
        elif p_command == APPICATION_COMMANDS.S_LOAD_ANALYTICS_WEEKLY:
            self.__m_crawl_controller.invoke_trigger(CRAWL_CONTROLLER_COMMANDS.S_LOAD_ANALYTICS_WEEKLY)
